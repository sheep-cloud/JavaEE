...
public class HelloJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        log.info("HelloJob.execute(context) jobKey : {}", JSON.toJSONString(jobKey));

        // 获取trigger
        TriggerKey triggerKey = context.getTrigger().getKey();
        log.info("HelloJob.execute(context) triggerKey : {}", JSON.toJSONString(triggerKey));

        // 获取jobDataMap
        JobDataMap jobDataMap = context.getJobDetail().getJobDataMap();
        JobDataMap triigerDataMap = context.getTrigger().getJobDataMap();
        JobDataMap mergedJobDataMap = context.getMergedJobDataMap();

        log.info("HelloJob.execute(context) jobDataMap : {}", JSON.toJSONString(jobDataMap));
        log.info("HelloJob.execute(context) triigerDataMap : {}", JSON.toJSONString(triigerDataMap));
        log.info("HelloJob.execute(context) mergedJobDataMap : {}", JSON.toJSONString(mergedJobDataMap));
